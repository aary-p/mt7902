                              ...
                              struct sk_buff **ret_skb)
{
    ...
    unsigned long expires;
    /* DEBUG: Initialize seq to -1 for clearer logging */
    int ret, seq = -1;
    ...
    dev_info(dev->dev, "MT7902: MCU_SEND: Enter cmd=0x%x, wait_resp=%d\n",
             cmd, wait_resp);
...
retry:
    /* DEBUG: Log parameters right before sending/retrying */
    dev_info(dev->dev, "MT7902: MCU_SEND: >>>> Sending cmd=0x%x (seq=%d, retry=%u, len=%d)\n",
             cmd, seq, retry, skb->len);
    ...
    do {
        ...
        if (!skb && !test_bit(MT76_MCU_RESET, &dev->phy.state) &&
            ...
            /* DEBUG: This is the retry log you asked for */
            dev_err(dev->dev, "MT7902: MCU_SEND: Retry message 0x%08x (seq %d), attempt %u\n",
                    cmd, seq, retry);
...
out:
    /* DEBUG: Log function exit and return value */
    dev_info(dev->dev, "MT7902: MCU_SEND: <<<< Exit cmd=0x%x, ret=%d\n",
             cmd, ret);
